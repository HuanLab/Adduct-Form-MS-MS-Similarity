...
for (adduct in adducts) {
  ...
  tryCatch({
    ...
    for (interval in ce_intervals) {
      ...
      for (i in seq_len(nrow(adduct_H))) {
        if (!is.na(adduct_H$WHOLE_COMPOUND_NAME[i]) && 
            !is.na(adduct_H$WHOLE_COLLISION_ENERGY[i])) {
          
          for (j in seq_len(nrow(adduct_other))) {
            if (adduct_H$WHOLE_COMPOUND_NAME[i] == adduct_other$WHOLE_COMPOUND_NAME[j] & adduct_H$INSTRUMENT_TYPE[i] == adduct_other$INSTRUMENT_TYPE[j]) {
              # Check the CE difference with the interval condition
              ce_diff <- (adduct_H$WHOLE_COLLISION_ENERGY[i] - adduct_other$WHOLE_COLLISION_ENERGY[j])
              
              if (ce_diff >= interval[1] & ce_diff < interval[2]) {
                pairs_list <- append(pairs_list, list(data.frame(
      ...
      if (length(pairs_list) > 0) {
        ...
        # Create unique filename based on interval
        filename <- paste0(adduct, "_pairs_", interval[1], "_to_", interval[2], ".csv")
        
        # Write pairs_df to CSV
        write.csv(pairs_df, file.path(output_dir, filename), row.names = FALSE)
        print(paste("Saved", nrow(pairs_df), "pairs to", file.path(output_dir, filename)))
      } else {
